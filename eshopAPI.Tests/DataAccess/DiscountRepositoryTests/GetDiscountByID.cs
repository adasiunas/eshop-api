    ...
    public class GetDiscountByID
    {
        ...
        public GetDiscountByID()
        {
            _options = new DbContextOptionsBuilder<ShopContext>()
                .UseInMemoryDatabase(databaseName: "GetDiscountByID")
                .Options;
        ...
        {
            Discount discount = await _repository.GetDiscountByID(_firstDiscountId);
            Assert.Equal(1, discount.ItemID);
        }

        [Fact]
        public async void Failure()
        {
            Discount discount = await _repository.GetDiscountByID(_firstDiscountId - 1);
